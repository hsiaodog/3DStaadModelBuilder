...
function snapAllGlobalJointsToPrecision() {
...
async function _run3DOneClickPipeline() {
  const md = Math.max(0, Math.min(6, Number.isFinite(state.measureDecimals) ? state.measureDecimals : 0));
  if (!confirm(
    `3D 一鍵處理 — 依序跑下列 9 步:\n\n` +
...
function _runFitMergeByPrecision(opts) {
  opts = opts || {};
  const skipConfirm = !!opts.skipConfirm;
  const md = Math.max(0, Math.min(6, Number.isFinite(state.measureDecimals) ? state.measureDecimals : 0));
  const _round = (v) => {
...
async function copyPageJointsMembersToSamePlanePage() {
...
async function consolidateAllPagesWithConfirm(opts) {
  opts = opts || {};
  const skipConfirm = !!opts.skipConfirm;
  const skipPushUndo = !!opts.skipPushUndo;
...
function findAllExtendableMembers(opts) {
  const all = [];
  for (const f of state.files) {
    const pages = f.pages || {};
...
function _zoomMainCanvasToRect(x1, y1, x2, y2, paddingFactor) {
  const r = wrap.getBoundingClientRect();
...
async function startExtendableMemberCheck() {
...
function _relayoutPageCore(p, opts) {
  ...
  // 使用拓樸分群(共享 joint)取代 midpoint 分群,確保連續柱 / 樑 不會因 X / Y 微漂移被拆群
  const hGroups = groupConnectedByJoint(horizontalsM, false);
  const vGroups = groupConnectedByJoint(verticalsM, true);
...
// 全局重排:逐頁套用同樣邏輯
async function relayoutNumberingAll(opts) {
  opts = opts || {};
  const skipConfirm = !!opts.skipConfirm;
  let totalPages = 0;
  for (const f of state.files) {
...
async function relayoutMembersNumberingAll(opts) {
  opts = opts || {};
  const skipConfirm = !!opts.skipConfirm;
  let totalPages = 0;
...
// ---------- 3D 立體預覽視窗 ----------
//   獨立浮動視窗:跳出後可拖移 / 縮放,跟主畫面同時運作
//   中鍵拖曳 = 旋轉、滾輪 = 縮放、Shift+中鍵 或 右鍵 = 平移、Hover = 詳情
//   切面拼減、視角預設(俯視 / 正面 / 側面 / 等軸)、顯示控制
export let _3dPreviewWindow: any = null;
function open3DPreviewDialog() {
...
function openMaterialMgrWindow() {
...
<style>
  * { box-sizing: border-box; }
    ...
    if (!filtered.length) {
      ...
      const _addRow = (m) => {
        ...
        btnDel.addEventListener("click", () => {
          ...
          _markDirty();
          _refresh();
          try { _searchMaterialDropdownRefresh && _searchMaterialDropdownRefresh(); } catch (_) {}
        });
  ...
  const _markDirty = () => {
    ...
    try { if (typeof projectDirty !== "undefined") projectDirty = true; } catch (_) {}
...
<style>
  * { box-sizing: border-box; }
  ...
  _searchWinAutofill = _autofillFromSelection;
...
function _renderSearchResults(div, doc, result, win) {
  div.innerHTML = "";
...
function showBusy(msg) {
  ...
  if (cb) { cb.classList.remove("active"); cb.onclick = null; }
}
...
function showBusyWithCancel(msg, onCancel) {
  showBusy(msg);
  const cb = document.getElementById("busyCancelBtn");
  if (!cb) return;
...
async function _saveRecentProject(name, handle) {
  if (!name) return;
  try {
    ...
    db.close();
    try { _refreshRecentProjectMenu && _refreshRecentProjectMenu(); } catch (_) {}
  } catch (e) {
    console.warn("[recent projects] save failed:", e);
  }
}
async function _getRecentProjects() {
  try {
    const db = await _openRecentDB();
...
async function _removeRecentProject(name) {
  try {
    const db = await _openRecentDB();
    const tx = db.transaction(_RECENT_STORE, "readwrite");
...
async function _openRecentProject(entry) {
  ...
  try {
    ...
    // 失敗的就從 recent 清掉
    try { await _removeRecentProject(name); _refreshRecentProjectMenu && _refreshRecentProjectMenu(); } catch (_) {}
  }
}

async function loadProjectFull(file, handle) {
  ...
  if (data.counters) {
    nextJointId  = data.counters.nextJointId  || 1;
    nextMemberId = data.counters.nextMemberId || 1;
    nextFileId   = data.counters.nextFileId   || 1;
    nextGlobalJointId = data.counters.nextGlobalJointId || 1;
    nextGlobalMemberId = data.counters.nextGlobalMemberId || 1;
  }
  ...
  if (Array.isArray(data.globalJoints)) {
    ...
    if (maxG + 1 > nextGlobalJointId) nextGlobalJointId = maxG + 1;
  ...
  if (Array.isArray(data.globalMembers)) {
    ...
    if (maxGM + 1 > nextGlobalMemberId) nextGlobalMemberId = maxGM + 1;
  } else {
    state.globalMembers = [];
  }
...
function _afterCalibrationChanged() {
  if (typeof invalidateRankCache === "function") invalidateRankCache();
  if (typeof inferAllGlobalJoints === "function") inferAllGlobalJoints();
  if (typeof refreshPageCoordSection === "function") refreshPageCoordSection();
  if (typeof refreshSectionLinkList === "function") refreshSectionLinkList();
...
let nextProjId = 1;
let projectDirty = false;    // 活躍專案自上次儲存後是否有變更
